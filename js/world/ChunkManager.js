...
export default class ChunkManager {
    ...
    async generateChunk(x, y) {
        ...
        // Register generated objects with WorldObjectManager
        if (this.world.worldObjectManager) {
            chunk.features.forEach(feature => this.world.worldObjectManager.registerObject(feature, chunkId));
            chunk.resources.forEach(resource => this.world.worldObjectManager.registerObject(resource, chunkId));
        } else {
            console.warn(`[ChunkManager] WorldObjectManager not found on world during chunk ${chunkId} generation.`);
        }
    ...
    async loadChunksAroundPosition(x, y) {
        ...
        for (const chunkId of toUnload) {
            ...
            // Remove objects from manager when unloading
            if (this.world.worldObjectManager) {
                this.world.worldObjectManager.removeObjectsForChunk(chunkId);
            }
        }
