export default class Player {
    ...
    onCollision(other) {
        ...
        if (other.type === 'resource') {
            // Request to collect the resource
            this.requestCollectResource(other);
            // NOTE: Actual resource removal and addition happens via network sync
        } else if (other.type === 'vehicle' && !this.insideVehicle) {
            // Check if can enter vehicle (e.g., using input)
            // if (this.game.input.isKeyDown('KeyE')) { // Assuming input is accessible here
    ...
    requestCollectResource(resource) {
        if (!resource || !resource.id || resource.collected) return; // Prevent double collection

        // Mark locally as collected to prevent repeated requests (temporary)
        resource.collected = true; 

        // Optimistically add resource locally (will be confirmed by network state)
        // Commenting this out - let's rely solely on network state update
        // this.addResource(resource.resourceType, resource.amount);

        // Tell the network to update the room state, removing the resource
        this.game.network.updateRoomState({
            resources: {
                [resource.id]: null // Set to null to signify deletion
            }
        });

        // Trigger a visual effect locally (optional, could also be triggered by network event)
        this.game.renderer.createEffect('collect', resource.x, resource.y, resource.color);

         // Optionally send a broadcast event for sound effects etc.
        ...
        this.game.debug.log(`Requested collection of resource: ${resource.id}`);
    }
